...
export default function AdminWallet() {
  ...
  async function load() {
    ...
    const list = data || [];
    list.sort((a: any, b: any) => (b.requested ? 1 : 0) - (a.requested ? 1 : 0) || Number(b.pts) - Number(a.pts));
    setRows(list);
  ...
  async function distribute() {
    ...
    if (!total) return;
    const earners = rows.filter((r) => r.is_monetized && r.is_verified && r.pts > 0);
    const totalPts = earners.reduce((a, r) => a + Number(r.pts), 0);
    if (!totalPts) return setMsg("No monetized users with points yet.");
    const supabase = createClient();
    for (const r of earners) {
      const share = Math.floor((Number(r.pts) / totalPts) * total);
      if (share > 0) await supabase.from("profiles").update({ wallet_balance: Number(r.balance) + share }).eq("id", r.uid);
    }
  ...
  async function resolvePayout(id: string, approve: boolean, userId: string, amount: number) {    const supabase = createClient();
    if (approve) {
  ...
  const earners = rows.filter((r) => r.is_monetized && r.is_verified && r.pts > 0);
  const totalPts = earners.reduce((a, r) => a + Number(r.pts), 0);
  const pendingUsers = rows.filter((r) => r.requested && !r.is_verified);
    ...
    <div className="p-4 pb-24 max-w-2xl mx-auto">
      ...
      <div className="glass-card p-5 rounded-2xl mb-6 border-2 border-green-300">
        ...
        {total > 0 && earners.length > 0 && (
          <div className="mt-3 space-y-1 text-xs text-gray-600">
            {earners.map((r) => (
              <p key={r.uid}>{r.name}: <b>₦{Math.floor((Number(r.pts) / totalPts) * total).toLocaleString()}</b> ({r.pts} pts)</p>
            ))}
          </div>
        )}
      ...
      {pendingUsers.length > 0 && (
        <>
          ...
          <div className="space-y-2 mb-8">
            {pendingUsers.map((r) => (
              <div key={r.uid} className="glass-card p-3 rounded-2xl flex items-center gap-3 border-2 border-amber-300">
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm truncate">📲 {r.name || "Farmer"}</p>
                  <p className="text-[10px] text-gray-500">{r.pts} pts — check their WhatsApp receipt & bank transfer</p>
                </div>
                <button onClick={() => setVerified(r.uid, true)} className="text-xs font-bold px-3 py-2 rounded-full bg-sky-600 text-white">✅ Verify</button>
              </div>
            ))}
          </div>
        </>
      ...
      <div className="space-y-2">
        {rows.map((r) => (
          <div key={r.uid} className="glass-card p-3 rounded-2xl flex items-center gap-3">
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm truncate">
                {r.name || "Farmer"} {r.is_verified && "✅"} {r.is_monetized && "💵"} {r.requested && !r.is_verified && "📲"}
              </p>
              <p className="text-[10px] text-gray-500">{r.pts} pts · wallet ₦{Number(r.balance).toLocaleString()}</p>
            </div>
            <button onClick={() => setVerified(r.uid, !r.is_verified)} className={`text-xs font-bold px-3 py-2 rounded-full ${r.is_verified ? "bg-gray-200 text-gray-700" : "bg-sky-600 text-white"}`}>
              {r.is_verified ? "Unverify" : "✅ Verify"}
            </button>
          </div>
        ))}
        {rows.length === 0 && <p className="text-sm text-gray-500">No members loaded — run the SQL above, then refresh.</p>}
      </div>
    </div>
  );
}