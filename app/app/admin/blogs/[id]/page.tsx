...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    ...
    setId(blogId);
    (async () => {
      const supabase = createClient();      const { data } = await supabase.from("blogs").select("*").eq("id", blogId).single();
      if (data) {
        setTitle(data.title || "");
  ...
  return (
    <div className="p-4 pb-24 max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSave} className="space-y-4">
        ...
        <textarea className="w-full p-3 rounded-xl border border-gray-200 bg-white/70" placeholder="SEO description (what Google shows under the title)" rows={2} value={seoDescription} onChange={(e) => setSeoDescription(e.target.value)} />
        ...
        </button>        <button type="button" onClick={handleDelete} className="w-full bg-red-600 text-white py-3 rounded-xl font-semibold">
          🗑️ Delete Post
        </button>
      </form>
    </div>
