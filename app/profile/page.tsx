...
export default function ProfilePage() {
  ...
  if (!user) return (    <div className="p-6 max-w-md mx-auto text-center pb-24">
      <div className="glass-card p-6 rounded-2xl space-y-3 mt-10">
        <p className="text-4xl">🌾</p>
        <p className="font-extrabold text-lg">Welcome to Farming Tech & Business</p>
        <p className="text-xs text-gray-500">Log in or create a free account to post, earn points & get paid monthly.</p>
        <Link href="/login" className="block w-full bg-green-600 text-white py-3 rounded-xl font-bold">🔓 Log In</Link>
        <Link href="/register" className="block w-full bg-forest-100 text-forest-700 py-3 rounded-xl font-bold">🎁 Create Free Account</Link>
      </div>
    </div>
  );
    ...
    <div className="pb-24 max-w-2xl mx-auto">
      ...
      <div className="px-4">
        <div className="flex items-end justify-between -mt-10">
          ...
          <button onClick={() => setEditing(!editing)} className="px-4 py-2 rounded-full text-sm font-bold bg-forest-100 text-forest-700 mb-1">✏️ Edit Profile</button>
        </div>

        <h1 className="text-xl font-extrabold mt-2">{me?.full_name || "Farmer"} {me?.verified && <span className="text-sky-500">✅</span>}</h1>
        <p className="text-xs text-gray-500">@{(me?.referral_code || user.id).slice(0, 8).toLowerCase()} · 📍 {me?.location || "Nigeria"}</p>        {me?.bio && <p className="text-sm text-gray-700 mt-1">{me.bio}</p>}
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          {me?.role === "admin" && (
            <Link href="/admin" className="text-xs font-bold bg-forest-800 text-white px-3 py-2 rounded-full">🛡️ Admin Dashboard</Link>
          )}
          ...
          <button onClick={logout} className="text-xs font-bold bg-red-100 text-red-700 px-3 py-2 rounded-full">🚪 Logout</button>
        </div>
        ...
        {editing && (
          <div className="glass-card p-4 rounded-2xl mt-4 space-y-2 border-2 border-forest-300">
            ...
            <textarea className={input} rows={2} placeholder="Bio — tell farmers about you & your farm" value={form.bio || ""} onChange={(e) => setForm({ ...form, bio: e.target.value })} />
