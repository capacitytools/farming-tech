...
If the image is not a plant or animal, set diagnosis to "Not a farm subject" and severity to "low".`;
    ...
    // Newest models first (new Google accounts need Gemini 3)
    const envModel = process.env.GEMINI_MODEL;
    const models = envModel
      ? [envModel, "gemini-3-flash", "gemini-3-flash-preview", "gemini-3-pro-preview", "gemini-2.5-flash"]
      : ["gemini-3-flash", "gemini-3-flash-preview", "gemini-3-pro-preview", "gemini-3-pro", "gemini-2.5-flash"];
    ...
    for (const model of models) {
      ...
      try {
        lastReason = JSON.parse(errText)?.error?.message || errText;
      } catch {
        lastReason = errText;
      }
      if (/API key/i.test(lastReason)) break;
    }

    if (!aiJson) return NextResponse.json({ error: "AI error: " + lastReason }, { status: 502 });
